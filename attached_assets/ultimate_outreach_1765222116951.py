...
def generate_instagram_dm(intel: BusinessIntelligence) -> str:
    ...
    templates = [
        ...
        f"Hey! Noticed {intel.business_name} is doing great ({intel.review_count}+ reviews 🔥). Quick q - are you handling all those booking inquiries manually or do you have something automated?",
        
        # Template 2: Pain + Curiosity
        f"Quick question about {intel.business_name} - how are you handling WhatsApp inquiries when you're busy with customers? Most {intel.category}s I talk to say it's their biggest headache.",
        
        # Template 3: Compliment + Hook
        f"Your spot looks incredible. Curious though - with that volume, how many booking requests do you think slip through when you can't respond fast enough?",
        
        # Template 4: Direct + Confident
        f"I help {intel.category}s in {intel.city} stop losing money to slow responses and no-shows. Saw {intel.business_name} and thought of a few things. Worth a quick chat?",
    ]
...
def generate_whatsapp_message(intel: BusinessIntelligence) -> str:
    ...
    return f"""Hola! Vi que {intel.business_name} tiene excelentes reseñas en Google.
...
def generate_email(intel: BusinessIntelligence) -> tuple[str, str]:
    ...
    body = f"""No voy a hacerte perder el tiempo.
...
def generate_followups(intel: BusinessIntelligence) -> tuple[str, str, str]:
    ...
    day3 = f"""Seguimiento rápido sobre {intel.business_name}.
...
— Phil"""
    ...
    day7 = f"""Una cosa más sobre {intel.business_name}.
...
— Phil"""
    ...
    day14 = f"""Última nota sobre esto.
...
<body>
    <div class="header">
        <h1>🎯 Ultimate Outreach System</h1>
        <p>Copy. Paste. Close. — Phil McGill</p>
        <div class="stats">
            <div class="stat">
